...
{
	qPathMgr::init();
	wstring strProjPath = qPathMgr::GetProjectPath();
	wstring strCppPath = strProjPath + L"\\Scripts\\qScriptMgr.cpp";
	wstring strHeaderPath = strProjPath + L"\\Scripts\\qScriptMgr.h";

	// 1. 현재 존재하는 모든 스크립트를 알아내야함.
	wstring strScriptIncludePath = qPathMgr::GetIncludePath();
	...
	fwprintf_s(pFile, L"class qScript;\n\n");

	fwprintf_s(pFile, L"class qScriptMgr\n{\n");
	fwprintf_s(pFile, L"public:\n\tstatic void GetScriptInfo(vector<wstring>& _vec);\n");
	fwprintf_s(pFile, L"\tstatic qScript * GetScript(const wstring& _strScriptName);\n");
	fwprintf_s(pFile, L"\tstatic qScript * GetScript(UINT _iScriptType);\n");
	fwprintf_s(pFile, L"\tstatic const wchar_t * GetScriptName(qScript * _pScript);\n};\n");
	...
	fwprintf_s(pFile, L"#include \"qScriptMgr.h\"\n\n");
	...
	// 세번째 함수
	fwprintf_s(pFile, L"qScript * qScriptMgr::GetScript(UINT _iScriptType)\n{\n");
	...
	// 네번째 함수
	fwprintf_s(pFile, L"const wchar_t * qScriptMgr::GetScriptName(qScript * _pScript)\n{\n");
